...
const ResonanceCheck: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-12 space-y-12 page-transition">
      {/* Header */}
      <div className="text-center space-y-4 relative">
        <div className="absolute inset-0 flex items-center justify-center opacity-6">
          <svg width="250" height="250" viewBox="0 0 250 250" className="fractal-particle">
            ...
            <circle cx="125" cy="125" r="70" fill="none" stroke="url(#resSlate)" strokeWidth="0.8"/>
            <circle cx="125" cy="125" r="45" fill="none" stroke="url(#resGold)" strokeWidth="0.6"/>
            <circle cx="125" cy="125" r="25" fill="none" stroke="url(#resSlate)" strokeWidth="0.4"/>
            <defs>
              <linearGradient id="resGold" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="#D4AF37"/>
                <stop offset="100%" stopColor="#C9A227"/>
              </linearGradient>
              <linearGradient id="resSlate" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <h2 className="text-4xl md:text-5xl font-display font-bold gradient-text relative z-10 mb-3">Resonantie Check</h2>
          ...
          {questions.map(q => (
            <div key={q.id} className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                <span className="text-xs text-[#708090]/70 w-28 text-right font-mono">{q.low}</span>
                <input
                  type="range"
                  min="0"
                  max="100"
                  ...
                  style={{
                    background: `linear-gradient(to right, 
                      ...
                      rgba(212, 175, 55, 0.3) ${answers[q.id]}%, 
                      rgba(5, 5, 5, 0.6) ${answers[q.id]}%, 
                      rgba(5, 5, 5, 0.6) 100%)`
                  }}
                />
                <span className="text-xs text-[#D4AF37]/80 w-28 text-left font-mono">{q.high}</span>
              </div>
            </div>
          ))}
        </div>

        <div className="pt-10 border-t border-[#708090]/20 flex flex-col items-center relative z-10">
          <div className="relative w-64 h-64 flex items-center justify-center">
            ...
            <div className="text-center z-10">
              <span className={`text-6xl font-bold font-mono ${avg > 70 ? 'gradient-text' : avg > 40 ? 'text-[#708090]' : 'text-red-400'}`}>
                {Math.round(avg)}%
              </span>
              <p className="text-[11px] text-[#708090]/70 uppercase tracking-widest mt-2 font-mono">Flow Index</p>
            </div>
          </div>

          <div className="mt-10 text-center max-w-lg">
            {avg > 70 ? (
              <div className="glass border border-[#D4AF37]/30 rounded-2xl p-6 luxury-glow-sm">
                <p className="text-[#D4AF37] font-semibold italic text-lg leading-relaxed">"Je resoneert sterk met het patroon. Behoud je koers via Wu Wei."</p>
              </div>
            ) : avg > 40 ? (
              <div className="glass border border-[#708090]/30 rounded-2xl p-6 luxury-glow-sm">
                <p className="text-[#708090] italic text-lg leading-relaxed">"Er is lichte frictie merkbaar. Onderzoek waar je vrije wil probeert af te wijken van je Dharma."</p>
              </div>
            ) : (
              <div className="glass border border-red-500/30 rounded-2xl p-6 luxury-glow-sm">
